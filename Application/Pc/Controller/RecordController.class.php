...

{
	...
	public function rebate(){
		...
		foreach ($num as $key => $val){
            ...
            $record=M($case)->where('num_id="'.session('num_id').'" AND state=1 AND time>='.$first_start.' AND time<='.$first_end.'')->select();
		...
		foreach ($num as $key => $val){
            ...
            $record=M($case)->where('num_id="'.session('num_id').'" AND state=1 AND time>='.$second_start.' AND time<='.$second_end.'')->select();
		...
		foreach ($num as $key => $val){
            ...
            $record=M($case)->where('num_id="'.session('num_id').'" AND state=1 AND time>='.$last_start.' AND time<='.$last_end.'')->select();
		...
		if(IS_AJAX){
			...
			foreach($datas['data'] as $key => $val){
				...
				if($val['cash']==1){
					...
					$money_USD=round($val['money']/$odds_money,2);
				...
				if($val['cash']==1){
					$str.='	<div class="in-list-text">'.$money.''.$money_type."(".$money_USD . " " . $money_type_USD .")" . '</div>';
				}else{
					$str.='	<div class="in-list-text">'.$money.''.$money_type . '</div>';
				}	
				...
				$str.='
					<div class="in-list-text">'.$name.'</div>
						...
						<div class="in-list-text in-berder">'.$hanging." ".$money_type .'</div>
